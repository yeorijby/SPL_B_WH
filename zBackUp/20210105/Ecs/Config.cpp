...
/*
	for (int i = 0; i < SC_CNT; i++ )	
	{
		...
		if(i == 0 || i == 1 || i == 2 || i == 5)	//스테커 교체 시 수정 
			continue;
...
//		m_bCvRetry[i]    = (UINT)GetPrivateProfileInt(szSection, "Retry", 1, ECS_INI_FILE);
	...
	for (i = 0; i < SC_CNT; i++ )
	{
		...
		GetPrivateProfileString(szSection, "Address", "", szBuff, 256, ECS_INI_FILE);
		m_sScPort[i].strAddress = szBuff;
		m_sScPort[i].nPort = GetPrivateProfileInt(szSection, "Port", 7000, ECS_INI_FILE);
		m_sScPort[i].bConnect  = (UINT)GetPrivateProfileInt(szSection, "Connect", 1, ECS_INI_FILE);
		m_sScPort[i].bRetry    = (UINT)GetPrivateProfileInt(szSection, "Retry", 1, ECS_INI_FILE);
		m_sScPort[i].dwReadTimeOut = GetPrivateProfileInt(szSection, "ReadTimeOut", 2000, ECS_INI_FILE);
	}
...
//		sprintf(szBuff, "%d", m_bCvRetry[i]);				WritePrivateProfileString(szSection, "Retry", szBuff, ECS_INI_FILE);
	...
	{
		sprintf(szSection, "Stacker%d", i+1);
		WritePrivateProfileString(szSection, "Address", (LPTSTR)(LPCTSTR)m_sScPort[i].strAddress,  ECS_INI_FILE);
		sprintf(szBuff, "%d", m_sScPort[i].nPort);
		WritePrivateProfileString(szSection, "Port", szBuff,  ECS_INI_FILE);
		sprintf(szBuff, "%d", m_sScPort[i].bConnect);
		WritePrivateProfileString(szSection, "Connect", szBuff, ECS_INI_FILE);
		sprintf(szBuff, "%d", m_sScPort[i].bRetry);
		WritePrivateProfileString(szSection, "Retry", szBuff, ECS_INI_FILE);
