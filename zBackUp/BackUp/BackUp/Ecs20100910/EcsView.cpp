...
void CEcsView::OnTimer(UINT nIDEvent) 
{
	...
	//TEST
/*	if (pDoc->m_pHostCl != NULL)
	{
//		pDoc->m_pHostCl->Arrived(3108);
		pDoc->m_pHostCl->Arrived2(3115, 3116);
	}
*/
...
BEGIN_EVENTSINK_MAP(CEcsView, CFormView)
	...
	ON_EVENT(CEcsView, IDC_CVCTRL914, 1 /* LBtnClick */, OnLBtnClickConveyor, VTS_I2)
	...
	ON_EVENT(CEcsView, IDC_CVCTRL1052, 1 /* LBtnClick */, OnLBtnClickConveyor, VTS_I2)
	ON_EVENT(CEcsView, IDC_CVCTRL1053, 1 /* LBtnClick */, OnLBtnClickConveyor, VTS_I2)
	ON_EVENT(CEcsView, IDC_CVCTRL1054, 1 /* LBtnClick */, OnLBtnClickConveyor, VTS_I2)
		...
		for (i = CV_2_START_TRACK;i < CV_2_START_TRACK + CV_2_TRACK_CNT && wParam == CV_2_PLC; i++)
		{
			if (pDoc->m_ConveyorTrackInfo[i].m_bModified)
			{
				...
				if(i >= 1049)
				{
					...
					if(pDoc->m_ConveyorTrackInfo[i].m_bOpMode)
						bOverLoad = FALSE;

					if(pDoc->m_ConveyorTrackInfo[i].m_bError)
						bOverLoad = TRUE;
				}
				
				pCv->SetValue(GetCvColor(bOverTime, bOverLoad, FALSE, (unsigned long)-1, TRUE, bPltSensor),
					  pDoc->m_ConveyorTrackInfo[i].m_bHeightErr,
					  pDoc->m_ConveyorTrackInfo[i].m_bStoStationReady,
					  pDoc->m_ConveyorTrackInfo[i].m_bRetStationReady,
					  pDoc->m_ConveyorTrackInfo[i].m_bPickingStationReady,
		...
		for (i = CV_7_START_TRACK;i < CV_7_START_TRACK + CV_7_TRACK_CNT && wParam == CV_7_PLC; i++)
		{
			if (pDoc->m_ConveyorTrackInfo[i].m_bModified)
			{
				...
				if(i > 117 && i < 122)
				{
					...
					if(pDoc->m_ConveyorTrackInfo[i].m_nDestPos == 15)
						nJobType = 4;
...
//	case	JOB_TYPE_MANUAL_PICKING:	return DARK_YELLOW;
//	case	JOB_TYPE_MANUAL_SITE:		return DARK_SITE;
	default	: 
		if (!bOpMode) 
			return DARK_GRAY;
		else 
...
void CEcsView::OnLBtnClickConveyor(short nID) 
{
	...
	if(nID < 900)
	{
		ConveyorDlg.m_nID = nID;
		ConveyorDlg.m_pDoc = GetDocument();
		ConveyorDlg.DoModal();
	}
	else
	{
...
*/
	...
	for (i = IDC_CVCTRL901; i < IDC_CVCTRL901 + CV_2_TRACK_CNT; i++)
	{
		...
		{
			strBttName.Format("SFA C/V Track #%d", nTrackNum);
			m_bttWnd.AddTool(GetDlgItem(i), strBttName);
		}
	}
