...
void CConveyor::ArriveDataCopy()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		int nBatchNo=11, nLineNum=11;

		if(!MOVEJOB->Add(pJobItem->m_nLuggNum, JOB_TYPE_STN_TO_STN, pJobItem->m_strLocation, nBatchNo, pJobItem->m_nPalletNo, pJobItem->m_nLevelNo, pJobItem->m_nStartPos, pJobItem->m_nDestPos, pJobItem->m_nPriority, 
								pJobItem->m_strProductID, pJobItem->m_strSecondLoc, JOB_STA_MOVE_CV_OPER_INVOKE, pJobItem->m_strGrade, "", 0, nLineNum))
			continue;
...
void CConveyor::CvDataChange8GroupTo7Group()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nComplete == 0 && 
			(m_pDoc->m_ConveyorTrackInfo[2002-1].m_nLuggNum == 0 || m_pDoc->m_ConveyorTrackInfo[2002-1].m_nDestPos == 0))
			continue;
