...
{
	if ( bStartPos == 100 || bStartPos == 101  )
...
BOOL CLib::IsValidHostDestPos(int bDestPos)
{
	...
	if (bDestPos == 200 || bDestPos == 201 || bDestPos == 202 || bDestPos == 203 || bDestPos == 251 )
		return TRUE;
...
CString CLib::GetCvErrorCode(int nErrorCode)
{
	...
	// 정복열 추가 (090821)
	case	61:				return "Stocker Up 비상";
	case	62:				return "Stocker Down 비상";
	case	63:				return "Stocker 10단 Over 감지";
	case	64:				return "Stocker 5단 Over 감지";
	...
	// 정복열 추가 (090821)
	case	101:			return "Stocker 펜스 도어 열림";
