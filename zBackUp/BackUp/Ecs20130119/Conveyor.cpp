...
int CConveyor::ReadStatus2()
{
	...
	if( Read(CMD_WORD_UNIT, RxBuff, DEVICE_CODE_D, 180, 5/*13*/) == FALSE )
	{
...
int CConveyor::ReadStatus3()
{
	...
	if( Read(CMD_WORD_UNIT, RxBuff, DEVICE_CODE_D, 400, 3) == FALSE )
	{
...
void CConveyor::ParsingCount(BYTE * pRxBuff)
{
	...
	if(pRxBuff[3] > 0)
	{
		m_pDoc->m_n5GroupInCount   = (pRxBuff[3] * 256) + pRxBuff[2]; // D400
	}
	else if(pRxBuff[3] == 0)
	{
		m_pDoc->m_n5GroupInCount   = pRxBuff[3] + pRxBuff[2]; // D400
	}

	if(pRxBuff[5] > 0)
	{
		m_pDoc->m_n6GroupInCount   = (pRxBuff[5] * 256) + pRxBuff[4]; // D400
	}
	else if(pRxBuff[5] == 0)
	{
		m_pDoc->m_n6GroupInCount   = pRxBuff[5] + pRxBuff[4]; // D400
	}
...
void CConveyor::StoManualInvokeCheck7()
{
	...
	for ( int i = 0; i < 1; i++ )
	{
		...
		if (WriteTrackInfo7(nTrackNum, pMoveJobItem->m_nLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, nSize))
		{
			...
			strLog.Format("StoManualInvokeCheck7.. 트랙[%d].. %s.. 수동 이동 지시..", nTrackNum+1, "수동입고대");
			LOG_JOB(LOG_POS_CV, pMoveJobItem->m_nLuggNum, strLog);
		...
		{
			strLog.Format("StoManualInvokeCheck7.. 트랙[%d].. %s.. 수동 이동 지시 실패..", nTrackNum+1, "수동입고대");
			LOG_ERROR(LOG_POS_CV, pMoveJobItem->m_nLuggNum, strLog);
		}
	}
}
		...
		case	808:
			{
				...
				/*	if(i == 6 && (TRACK_INFO[794-1].m_nComplete == 1 || TRACK_INFO[793-1].m_nComplete == 1))
					{
						if(TRACK_INFO[798-1].m_nLuggNum == 0 && TRACK_INFO[798-1].m_bPltSensor == FALSE &&
						   TRACK_INFO[799-1].m_nLuggNum == 0 && TRACK_INFO[799-1].m_bPltSensor == FALSE &&
						   TRACK_INFO[800-1].m_nLuggNum == 0 && TRACK_INFO[800-1].m_bPltSensor == FALSE &&
						   TRACK_INFO[801-1].m_nLuggNum == 0 && TRACK_INFO[801-1].m_bPltSensor == FALSE &&

						   TRACK_INFO[803-1].m_nLuggNum == 0 && TRACK_INFO[803-1].m_bPltSensor == FALSE &&
						   TRACK_INFO[804-1].m_nLuggNum == 0 && TRACK_INFO[804-1].m_bPltSensor == FALSE &&
						   TRACK_INFO[805-1].m_nLuggNum == 0 && TRACK_INFO[805-1].m_bPltSensor == FALSE &&
						   TRACK_INFO[806-1].m_nLuggNum == 0 && TRACK_INFO[806-1].m_bPltSensor == FALSE)
						{
							MOVEJOB->FindChange(TRACK_INFO[794-1].m_nLuggNum, 0, JOB_STA_MOVE_RGV_OPER_REQUEST);

							MOVEJOB->FindChange(TRACK_INFO[793-1].m_nLuggNum, 0, JOB_STA_MOVE_RGV_OPER_REQUEST);
						}
					}
				*/
				...
				if(TRACK_INFO[808-1].m_nDestPos == 22)//B Line
				{
					...
					//if(i == 6 && TRACK_INFO[796-1].m_nDestPos == 22 && TRACK_INFO[796-1].m_nComplete == 1)
