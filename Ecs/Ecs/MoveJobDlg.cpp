...
BEGIN_MESSAGE_MAP(CMoveJobDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_COUNT_CHOICE_RESET, OnButtonCountChoiceReset)
...
void CMoveJobDlg::OnButtonCountChoiceReset() 
{
    ...
    GetDlgItem(IDC_EDIT_JOB_CHOICE_LINE)->GetWindowText(strTemp);
	nLineNum = atoi(strTemp);

    GetDlgItem(IDC_EDIT_JOB_CHOICE_COUNT)->GetWindowText(strTemp);
	nLineCount = atoi(strTemp);

	strTemp2.Format("선택한 Line#%d 카운트 숫자 %d로 변경 하시겠습니까?", nLineNum, nLineCount);

	if (AfxMessageBox(strTemp2, MB_OKCANCEL) == IDCANCEL)
		return ;

	m_pDoc->m_nSetLineCount[nLineNum-1] = nLineCount;
