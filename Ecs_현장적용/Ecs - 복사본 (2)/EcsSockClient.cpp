...
void CEcsSockClient::EcsClientSendData()
{
	...
	//sprintf(TxBuff, "%c%20s%08d%50s%08s%05s%08s%c", 
	//	STX, m_pDoc->m_strCodeNum, m_pDoc->m_nPeriodNum, m_pDoc->m_strProdName, m_pDoc->m_strQuantity, m_pDoc->m_strLineNum, m_pDoc->m_strCheckNum,ETX);

	sprintf(TxBuff, "%c%01d%20s%08d%50s%08s%05s%08s%c", 
		STX, 1, m_pDoc->m_strCodeNum, m_pDoc->m_nPeriodNum, m_pDoc->m_strProdName, m_pDoc->m_strQuantity, m_pDoc->m_strLineNum, m_pDoc->m_strCheckNum,ETX);

	CString strLog;

	//if(SendMsg(TxBuff, 101) == FALSE)
	if(SendMsg(TxBuff, 102) == FALSE)
	{
...
void CEcsSockClient::EcsClientSendData2()
{
	char TxBuff[256];
	int nSeqNo = 0;
	memset(TxBuff, 0x0, sizeof(TxBuff));
	...
	sprintf(TxBuff, "%c%01d%20s%08d%50s%08s%05s%08s%c", 
		STX, 2, m_pDoc->m_strCodeNum2, m_pDoc->m_nPeriodNum2, m_pDoc->m_strProdName2, m_pDoc->m_strQuantity2, m_pDoc->m_strLineNum2, m_pDoc->m_strCheckNum2,ETX);

	CString strLog;

	if(SendMsg(TxBuff, 102) == FALSE)
	{
		strLog.Format("EcsClientSendData2 송신 실패.. 에러=[%s]", CLib::GetSystemErrMsg());
		LOG_ERROR(LOG_POS_HOST, 0, strLog);
		return;
	}

	strLog.Format("EcsClientSendData2 송신.. 성공..");
	LOG_JOB(LOG_POS_HOST, 0, strLog);
