...
//		return CString("출하분류대");
	...
	case	IMS_ECS_RET_STATION_256:
	case	IMS_ECS_RET_STATION_257:
	...
	for (pos = m_JobList.GetHeadPosition(); pos != NULL;)
	{
			...
			pJobItem->m_nJobType == JOB_TYPE_RACK_TO_RACK)
		{
			switch(pJobItem->m_nDestPos)
			{
				...
				case	IMS_ECS_RET_STATION_255:	
				case	IMS_ECS_RET_STATION_256:
				case	IMS_ECS_RET_STATION_257:	nGroup = 16; break;
			}
		}
...
//	m_csJobAdd.Lock();
	...
	if (pJobItem->m_nStatus == JOB_STA_NEW)		
		Invoke(pJobItem);
...
//	m_csJobAdd.Lock();
	...
	for (pos = m_JobList.GetHeadPosition(); pos != NULL;)
	{
		...
		else if(pJobItem->m_nDestPos == IMS_ECS_RET_STATION_203 && m_pDoc->m_b6groupCheck == TRUE)
		{
			...
			(m_pDoc->m_b7groupCheck == TRUE || 
			(m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[8] != 15 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[9] != 15 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[10] != 15 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[11] != 15 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[12] != 15 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[13] != 15)))
			continue;
		else if(pJobItem->m_nDestPos == IMS_ECS_RET_STATION_256 && 
			(m_pDoc->m_b8groupCheck == TRUE || 
			(m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[8] != 21 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[9] != 21 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[10] != 21 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[11] != 21 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[12] != 21 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[13] != 21)))
			continue;
		else if(pJobItem->m_nDestPos == IMS_ECS_RET_STATION_257 && 
			(m_pDoc->m_b9groupCheck == TRUE || 
			(m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[8] != 22 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[9] != 22 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[10] != 22 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[11] != 22 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[12] != 22 &&
			m_pDoc->m_ConveyorThreadInfo[CV_9_PLC].m_ConveyorTroubleMsg.nD[13] != 22)))
			continue;
		//
