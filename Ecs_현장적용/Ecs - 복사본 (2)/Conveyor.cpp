...
{ 
	...
	m_nArrivedLuggNum5 = 0;
	m_nArrivedLuggNum6 = 0;

	m_nLineCountLuggNum = 0;
	m_nLineCountLuggNum3 = 0;
	m_nLineCountLuggNum4 = 0;
	m_nLineCountLuggNum5 = 0;
	m_nLineCountLuggNum6 = 0;
...
int CConveyor::ReadStatus1()
{
    ...
    if( Read(CMD_WORD_UNIT, RxBuff, DEVICE_CODE_D, 160, 22) == FALSE )
	{
...
int CConveyor::ReadStatus9()
{
	...
	memset(RxBuff, 0x0, sizeof(RxBuff));
	if( Read(CMD_WORD_UNIT, RxBuff, DEVICE_CODE_D, 10, 720) == FALSE ) 
	{
		strLog.Format("ReadStatus9.. CV#%d 상태정보 수신 실패.. [%s]", m_nNum+1, m_strErrMsg);
		LOG_ERROR(LOG_POS_CV, LOG_SYSTEM, strLog);
		return FALSE;
	}
	ParsingFrame9(RxBuff, 2100, 72);

	if( Read(CMD_WORD_UNIT, RxBuff, DEVICE_CODE_D, 730, 720) == FALSE ) 
	{
		strLog.Format("ReadStatus9.. CV#%d 상태정보 수신 실패.. [%s]", m_nNum+1, m_strErrMsg);
		LOG_ERROR(LOG_POS_CV, LOG_SYSTEM, strLog);
		return FALSE;
	}
	ParsingFrame9(RxBuff, 2172, 72);

	if( Read(CMD_WORD_UNIT, RxBuff, DEVICE_CODE_D, 1450, 720) == FALSE ) 
	{ 
		strLog.Format("ReadStatus9.. CV#%d 상태정보 수신 실패.. [%s]", m_nNum+1, m_strErrMsg);
		LOG_ERROR(LOG_POS_CV, LOG_SYSTEM, strLog);
		return FALSE;
	}
	ParsingFrame9(RxBuff, 2244, 72);

	if( Read(CMD_WORD_UNIT, RxBuff, DEVICE_CODE_D, 2170, 720) == FALSE ) //D2890까지 읽음
	{
		strLog.Format("ReadStatus9.. CV#%d 상태정보 수신 실패.. [%s]", m_nNum+1, m_strErrMsg);
		LOG_ERROR(LOG_POS_CV, LOG_SYSTEM, strLog);
		return FALSE;
	}
	ParsingStatus9(RxBuff); //D2500부터 상태값 쓰고 읽기 
...
void CConveyor::ParsingFrame9(BYTE *pRxBuff, int nFromTrackNum, int nCnt)
{
	int	nTrackNum = nFromTrackNum;
	...
	for ( int i = 0; i < nCnt; i++ ) 
	{
 		...
 		if (TRACK_INFO[nTrackNum].m_nLuggNum2 != nLuggNum2)
		{
			...
			if(nTrackNum == 2122-1 || nTrackNum == 2132-1 || nTrackNum == 2142-1 || nTrackNum == 2152-1 || nTrackNum == 2162-1 || nTrackNum == 2172-1 || nTrackNum == 2182-1 ||
			   nTrackNum == 2192-1 || nTrackNum == 2202-1 || nTrackNum == 2212-1 || nTrackNum == 2222-1 || nTrackNum == 2232-1 || nTrackNum == 2242-1 || nTrackNum == 2252-1)
			{
				if(nLuggNum != 0 && nLuggNum2 != 0)
				{
					...
					if(pMoveJobItem != NULL)
					{
						CMoveJobItem *pMoveJobItem2 = MOVEJOB->Find(nLuggNum2);
							...
							{
								if(pMoveJobItem->m_strProductID == pMoveJobItem2->m_strProductID)
								{
									pMoveJobItem->m_nErrCode = nLuggNum2;
									pMoveJobItem->m_nLastPallet = pMoveJobItem2->m_nLastPallet;

									MOVEJOB->Remove(pMoveJobItem2);
								}
...
void CConveyor::ParsingStatus9(BYTE *pRxBuff)
{
	CString strLog;
	BOOL bStatus[14];

	WORD wWordA = SwapToWord(pRxBuff+660);


	for (int i = 0; i < 14; i++ )
	{
		bStatus[i] = CLib::IsSet(wWordA, i);
		if(bStatus[i] ^ m_pDoc->m_bLineGoing[i])
		{
			m_pDoc->m_bLineGoing[i] = bStatus[i];
		}

		m_pDoc->m_nSetLine2[i] = SwapToWord(pRxBuff + 780 + i*2);
	}
}
...
/*	BOOL bStatus = CLib::IsSet(wWordA, 5); //외주창고 입출고 Mode
	...
	m_pDoc->m_nLineModeAB = pRxBuff[42]; //D181
}
...
void CConveyor::ArrivedCheck1()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		if ( nCurLuggNum == m_nArrivedLuggNum1 )			
			continue;
 ...
 void CConveyor::ArrivedCheck2()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		if ( nCurLuggNum == m_nArrivedLuggNum2 )			
			continue;
...
void CConveyor::ArrivedCheck5()
{
	CString strLog, strTemp="출고: ";

	for (int i=0; i<1; i++)
	{
		//int nTrackNum = 2300 - 1;
		int nTrackNum = 2301 - 1;
		int nCurLuggNum = TRACK_INFO[nTrackNum].m_nLuggNum;

		if (nCurLuggNum == 0) m_nArrivedLuggNum5 = 0;
		if (nCurLuggNum == m_nArrivedLuggNum5)			
			continue;
		...
		if (pMoveJobItem->m_strProductID != m_pDoc->m_strCodeNum2)
		{
			if (TRACK_INFO[2302-1].m_nLuggNum != 0)
				continue;
		}

		if (pMoveJobItem->m_nJobType == JOB_TYPE_STN_TO_STN)
		{
			m_pDoc->m_pLog->Job(LOG_POS_SYSTEM, nCurLuggNum, "Job Complete5.. 외주 출고대 도착5..");
			
			pMoveJobItem->m_nStatus = JOB_STA_COMPLETE;

			m_pDoc->m_strCodeNum2 = "";
			m_pDoc->m_nPeriodNum2 = 0;
			m_pDoc->m_strProdName2 = "";
			m_pDoc->m_strQuantity2 = ""; //수량 
			m_pDoc->m_strLineNum2 = "";  //생산라인 
			m_pDoc->m_strCheckNum2 = ""; //전표번호 

			m_pDoc->m_strCodeNum2 = pMoveJobItem->m_strProductID;
			m_pDoc->m_nPeriodNum2 = pMoveJobItem->m_nPriority;
			m_pDoc->m_strProdName2 = pMoveJobItem->m_strGrade;
			m_pDoc->m_strQuantity2 = pMoveJobItem->m_strQuantity; //수량 
			m_pDoc->m_strLineNum2 = pMoveJobItem->m_strLocation;  //생산라인 
			m_pDoc->m_strCheckNum2 = pMoveJobItem->m_strSecondLoc; //전표번호 

			if(pMoveJobItem->m_nLineNum == 15)
			{
				m_pDoc->m_strCodeNum2 = "";
				m_pDoc->m_nPeriodNum2 = 0;
				m_pDoc->m_strProdName2 = "";
				...
				m_pDoc->m_strCodeNum2 = pMoveJobItem->m_strProductID;
				m_pDoc->m_nPeriodNum2 = pMoveJobItem->m_nHeight;

				pMoveJobItem->m_strGrade.TrimRight();
				strTemp += pMoveJobItem->m_strGrade;
				m_pDoc->m_strProdName2 = strTemp;
			}
			...
			if(m_pDoc->m_pHostCl != NULL) 
			{
				if(pMoveJobItem->m_nLineNum != 15)
				{
					...
					if(TRACK_INFO[nTrackNum].m_nLuggNum !=0 && TRACK_INFO[nTrackNum].m_nLuggNum2 != 0)
						m_pDoc->m_pHostCl->Arrived3(nCurLuggNum, TRACK_INFO[nTrackNum].m_nLuggNum2);
				}
			...
			if ( m_pDoc->m_pEcsCl != NULL)
				m_pDoc->m_pEcsCl->EcsClientSendData2();

			m_nArrivedLuggNum5 = nCurLuggNum;
		}
		else if(pMoveJobItem->m_nJobType == JOB_TYPE_MANUAL_STN_TO_STN)
		{
			m_pDoc->m_strCodeNum2 = "";
			...
			m_pDoc->m_pLog->Job(LOG_POS_SYSTEM, nCurLuggNum, "수동 Job Complete..외주 출고대 도착5..");
			//m_pDoc->m_pJob->m_nResultCount[pJobItem->GetStackerNum()]++;
			//::PostMessage(m_pDoc->m_hwndView, WM_USER_JOB_NOTIFY, JOB_RESULT_DRAW, 0);
			MOVEJOB->Remove(pMoveJobItem);
...
void CConveyor::ArrivedCheck6()
{
	...
	{
		//int nTrackNum = 2300 - 1;
		int nTrackNum = 2302 - 1;
		int nCurLuggNum = TRACK_INFO[nTrackNum].m_nLuggNum;

		if ( nCurLuggNum == 0 ) m_nArrivedLuggNum6 = 0;
		if ( nCurLuggNum == m_nArrivedLuggNum6 )			
			continue;
		...
		if (pMoveJobItem->m_nJobType == JOB_TYPE_STN_TO_STN)
		{
			m_pDoc->m_pLog->Job(LOG_POS_SYSTEM, nCurLuggNum, "Job Complete.. 외주 출고대 도착6..");
			...
			m_nArrivedLuggNum6 = nCurLuggNum;
		}
		else if(pMoveJobItem->m_nJobType == JOB_TYPE_MANUAL_STN_TO_STN)
		{
			m_pDoc->m_pLog->Job(LOG_POS_SYSTEM, nCurLuggNum, "수동 Job Complete..외주 출고대 도착6..");
			//m_pDoc->m_pJob->m_nResultCount[pJobItem->GetStackerNum()]++;
			//::PostMessage(m_pDoc->m_hwndView, WM_USER_JOB_NOTIFY, JOB_RESULT_DRAW, 0);
			MOVEJOB->Remove(pMoveJobItem);
...
*/
void CConveyor::ArrivedDiscountCheck()
{
	CString strLog;

	//int nTrackNumber[]  = {2128, 2138, 2148, 2158, 2168, 2178, 2188, 2198, 2208, 2218, 2228, 2238, 2248, 2258};
	int nTrackNumber[]  = {2260, 2261, 2262, 2263, 2264, 2265, 2266, 2267, 2268, 2269, 2270, 2271, 2272, 2273};

	for (int i=0; i<14; i++)
	{
		int nTrackNum = nTrackNumber[i]-1;
		int nCurLuggNum = TRACK_INFO[nTrackNum].m_nLuggNum;
		int nCurLuggNum2 = TRACK_INFO[nTrackNum].m_nLuggNum2;
		int nStartPos = TRACK_INFO[nTrackNum].m_nStartPos;
		int nComparePos = i+1;

		if ( nCurLuggNum == 0 ) m_nArrivedDiscountLugg[i] = 0;
		if ( nCurLuggNum == m_nArrivedDiscountLugg[i] )			
			continue;

		if(nStartPos != nComparePos)
			continue;

		if(nCurLuggNum != 0 && nCurLuggNum2 != 0)
			m_pDoc->m_nSetLineCount2[i] = m_pDoc->m_nSetLineCount2[i] - 2;

		if(nCurLuggNum != 0 && nCurLuggNum2 == 0)
			m_pDoc->m_nSetLineCount2[i] = m_pDoc->m_nSetLineCount2[i] - 1;

		CMoveJobItem *pMoveJobItem = MOVEJOB->Find(nCurLuggNum);
		if(pMoveJobItem != NULL)
			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_PALLET_OPER_INVOKE;

		m_nArrivedDiscountLugg[i] = nCurLuggNum;
...
void CConveyor::LineCountCheck3()
{
	CString strLog;
	int nLineQuantity=0;
	...
	for (int i=0; i<1; i++)
	{
		...
		if(nCurLuggNum == 0) m_nLineCountLuggNum3 = 0;
		if(nCurLuggNum == m_nLineCountLuggNum3)			
			continue;
		...
		//임시방편
		if(m_pDoc->m_bSetTestCheck == FALSE)
		{
			bNotFullCheck = TRUE;
			if(bNotFullCheck)
			{
				nDestPos = 8;
				if(WriteTrackInfo7(nTrackNum, nCurLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, 0))
				{
					m_nLineCountLuggNum3 = nCurLuggNum;

					strLog.Format("WriteTrackInfo7LineCountCheck3.. CV#%d 도착지[%d] 쓰기 성공..!", nTrackNum+1, nDestPos);
					LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
				...
				{
					strLog.Format("WriteTrackInfo7LineCountCheck3.. CV#%d 도착지[%d] 쓰기 실패..!", nTrackNum+1, nDestPos);
		...
		if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[0] && m_pDoc->m_nSetLineCount2[0] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[0], 1, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[0] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[0], 1, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[0]++;
			pMoveJobItem->m_nBatchNo = 1;
			nDestPos = 1;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[0]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[1] && m_pDoc->m_nSetLineCount2[1] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[1], 2, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[1] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[1], 2, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[1]++;
			pMoveJobItem->m_nBatchNo = 2;
			nDestPos = 2;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[1]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[2] && m_pDoc->m_nSetLineCount2[2] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[2], 3, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[2] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[2], 3, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[2]++;
			pMoveJobItem->m_nBatchNo = 3;
			nDestPos = 3;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[2]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[3] && m_pDoc->m_nSetLineCount2[3] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[3], 4, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[3] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[3], 4, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[3]++;
			pMoveJobItem->m_nBatchNo = 4;
			nDestPos = 4;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[3]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[4] && m_pDoc->m_nSetLineCount2[4] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[4], 5, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[4] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[4], 5, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[4]++;
			pMoveJobItem->m_nBatchNo = 5;
			nDestPos = 5;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[4]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[5] && m_pDoc->m_nSetLineCount2[5] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[5], 6, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[5] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[5], 6, nCurLuggNum);
			m_pDoc->m_nSetLineCount2[5]++;
			pMoveJobItem->m_nBatchNo = 6;
			nDestPos = 6;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[5]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[6] && m_pDoc->m_nSetLineCount2[6] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[6], 7, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[6] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[6], 7, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[6]++;
			pMoveJobItem->m_nBatchNo = 7;
			nDestPos = 7;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[6]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[7] && m_pDoc->m_nSetLineCount2[7] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[7], 8, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[7] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[7], 8, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[7]++;
			pMoveJobItem->m_nBatchNo = 8;
			nDestPos = 8;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[7]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[8] && m_pDoc->m_nSetLineCount2[8] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[8], 9, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[8] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[8], 9, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[8]++;
			pMoveJobItem->m_nBatchNo = 9;
			nDestPos = 9;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[8]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[9] && m_pDoc->m_nSetLineCount2[9] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[9], 10, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[9] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[9], 10, nCurLuggNum);
			m_pDoc->m_nSetLineCount2[9]++;
			pMoveJobItem->m_nBatchNo = 10;
			nDestPos = 10;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[9]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[10] && m_pDoc->m_nSetLineCount2[10] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[10], 11, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[10] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[10], 11, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[10]++;
			pMoveJobItem->m_nBatchNo = 11;
			nDestPos = 11;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[10]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[11] && m_pDoc->m_nSetLineCount2[11] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[11], 12, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[11] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[11], 12, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[11]++;
			pMoveJobItem->m_nBatchNo = 12;
			nDestPos = 12;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[11]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[12] && m_pDoc->m_nSetLineCount2[12] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[12], 13, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[12] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[12], 13, nCurLuggNum); 
			m_pDoc->m_nSetLineCount2[12]++;
			pMoveJobItem->m_nBatchNo = 13;
			nDestPos = 13;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[12]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[13] && m_pDoc->m_nSetLineCount2[13] < 16 && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[13], 14, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
				continue;

			//m_pDoc->m_nSetLineCount2[13] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[13], 14, nCurLuggNum);
			m_pDoc->m_nSetLineCount2[13]++;
			pMoveJobItem->m_nBatchNo = 14;
			nDestPos = 14;

			pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

			strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[13]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
...
/*			if(nLineQuantity > 1)
			{
				for(i=0; i<14; i++)
				{
					if(pMoveJobItem->m_nLineNum != m_pDoc->m_nSetLine2[i])
  					...
  					if(m_pDoc->m_nSetLineCount2[i] >= 16) //위에 상황에서 12개로 셋팅하고 여기에서는 16개를 본다 
						continue;

					if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[i])
					{
						//다른 제품이 한라인에 섞이지 않게..
						CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[i], i+1, pMoveJobItem->m_strProductID);
						if(pMoveLastJobItem != NULL)								
							continue;

						//m_pDoc->m_nSetLineCount2[i] = MOVEJOB->BufferLineCountCheck(m_pDoc->m_nSetLine2[i], i+1, nCurLuggNum);
						m_pDoc->m_nSetLineCount2[i]++;
						pMoveJobItem->m_nBatchNo = i+1;
						nDestPos = i+1;

						pMoveJobItem->m_nStatus = JOB_STA_MOVE_CV_BUFFER_OPER_INVOKE;

						strLog.Format("LineCountCheck3.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[i]);
						LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
...
void CConveyor::LineCountCheck4()
{
	CString strLog;
	int nLineQuantity=0;
	...
	for (int i=0; i<1; i++)
	{
		...
		if(nCurLuggNum == 0) m_nLineCountLuggNum4 = 0;
		if(nCurLuggNum == m_nLineCountLuggNum4)			
			continue;
		...
		if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[0] && m_pDoc->m_nSetLineCount2[0] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[0]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[1] && m_pDoc->m_nSetLineCount2[1] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[1]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[2] && m_pDoc->m_nSetLineCount2[2] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[2]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[3] && m_pDoc->m_nSetLineCount2[3] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[3]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[4] && m_pDoc->m_nSetLineCount2[4] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[4]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[5] && m_pDoc->m_nSetLineCount2[5] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[5]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[6] && m_pDoc->m_nSetLineCount2[6] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[6]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[7] && m_pDoc->m_nSetLineCount2[7] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[7]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[8] && m_pDoc->m_nSetLineCount2[8] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[8]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[9] && m_pDoc->m_nSetLineCount2[9] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[9]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[10] && m_pDoc->m_nSetLineCount2[10] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[10]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[11] && m_pDoc->m_nSetLineCount2[11] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[11]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[12] && m_pDoc->m_nSetLineCount2[12] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[12]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[13] && m_pDoc->m_nSetLineCount2[13] < 16 && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[13]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
...
/*			if(nLineQuantity > 1)
			{
				for(i=0; i<14; i++)
				{
					...
					if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[i])
					{
						...
						strLog.Format("LineCountCheck4.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[i]);
						LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
  ...
  //Exit:
		...
		if(bNotFullCheck)
		{
			if(WriteTrackInfo7(nTrackNum, nCurLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, 0))
			{
				m_nLineCountLuggNum4 = nCurLuggNum;

				strLog.Format("WriteTrackInfo7LineCountCheck4.. CV#%d 도착지[%d] 쓰기 성공..!", nTrackNum+1, nDestPos);
				LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
			}
			else
			{
				strLog.Format("WriteTrackInfo7LineCountCheck4.. CV#%d 도착지[%d] 쓰기 실패..!", nTrackNum+1, nDestPos);
				LOG_ERROR(LOG_POS_CV, nCurLuggNum, strLog);
			}
		}
	}
...
void CConveyor::LineCountCheck5()
{
	CString strLog;
	int nLineQuantity=0;
	...
	for (int i=0; i<1; i++)
	{
		...
		if(nCurLuggNum == 0) m_nLineCountLuggNum5 = 0;
		if(nCurLuggNum == m_nLineCountLuggNum5)			
			continue;
		...
		if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[0] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[0], 1, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[0]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[1] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[1], 2, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[1]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[2] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[2], 3, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[2]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[3] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[3], 4, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[3]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[4] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[4], 5, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[4]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[5] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[5], 6, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[5]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[6] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[6], 7, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[6]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[7] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[7], 8, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[7]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[8] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[8], 9, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[8]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[9] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[9], 10, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[9]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[10] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[10], 11, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[10]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[11] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[11], 12, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[11]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[12] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[12], 13, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[12]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);

			bNotFullCheck = TRUE;
		}
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[13] && nLineQuantity > 1)
		{
			//다른 제품이 한라인에 섞이지 않게..
			CMoveJobItem *pMoveLastJobItem = MOVEJOB->BufferLineLastCheck(m_pDoc->m_nSetLine2[13], 14, pMoveJobItem->m_strProductID);
			if(pMoveLastJobItem != NULL)								
			...
			strLog.Format("LineCountCheck5.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[13]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
			...
			if(nLineQuantity == 1)
			{
				for(int i=0; i<14; i++)
				{
					...
					if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[i])
					{
						...
						strLog.Format("LineCountCheck.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[i]);
						LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		if(bNotFullCheck)
		{
			...
			{
				m_nLineCountLuggNum5 = nCurLuggNum;

				strLog.Format("WriteTrackInfo7LineCountCheck5.. CV#%d 도착지[%d] 쓰기 성공..!", nTrackNum+1, nDestPos);
				LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
			}
			else
			{
				strLog.Format("WriteTrackInfo7LineCountCheck5.. CV#%d 도착지[%d] 쓰기 실패..!", nTrackNum+1, nDestPos);
				LOG_ERROR(LOG_POS_CV, nCurLuggNum, strLog);
			}
		}
	}
...
void CConveyor::LineCountCheck6()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[0] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[0]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[1] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[1]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[2] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[2]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[3] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[3]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[4] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[4]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[5] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[5]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[6] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[6]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[7] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[7]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[8] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[8]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[9] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[9]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[10] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[10]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[11] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[11]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[12] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[12]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		else if(pMoveJobItem->m_nLineNum == m_pDoc->m_nSetLine2[13] && nLineQuantity > 1)
		{
			...
			strLog.Format("LineCountCheck6.. 카운트 체크.. ImsBuffer=[%d], BufferLine=[%d], Count=[%d] ..", pMoveJobItem->m_nLineNum, pMoveJobItem->m_nBatchNo, m_pDoc->m_nSetLineCount2[13]);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
		...
		if(bNotFullCheck)
		{
			if(WriteTrackInfo(nTrackNum, nCurLuggNum, nJobType, nStartPos, nDestPos, nComplete))
			{
				m_nLineCountLuggNum6 = nCurLuggNum;

				strLog.Format("WriteTrackInfo..LineCountCheck6.. CV#%d 도착지[%d] 쓰기 성공..!", nTrackNum+1, nDestPos);
				LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
			}
			else
			{
				strLog.Format("WriteTrackInfo..LineCountCheck6.. CV#%d 도착지[%d] 쓰기 실패..!", nTrackNum+1, nDestPos);
				LOG_ERROR(LOG_POS_CV, nCurLuggNum, strLog);
			}
		}
	}
...
void CConveyor::ArriveCountCheck()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		else if(pJobItem->m_nDestPos == IMS_ECS_RET_STATION_255 || pJobItem->m_nDestPos == IMS_ECS_RET_STATION_256 ||
			pJobItem->m_nDestPos == IMS_ECS_RET_STATION_257)
		{
//			m_pDoc->m_n7GroupInternalBatch = pJobItem->m_nBatchNo;
//			m_pDoc->m_n7GroupInternalPallet = pJobItem->m_nPalletNo;

			strLog.Format("Change Status.. Counter..트랙[%d]..출하이동대..", nTrackNum+1);
			LOG_JOB(LOG_POS_CV, nCurLuggNum, strLog);
/*
...
void CConveyor::ArriveDataCopy()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_bSetTestCheck == TRUE) //임시방편
		{
			nBatchNo=0;
			nLastPallet=0;
			if (pJobItem->m_nDestPos == IMS_ECS_RET_STATION_255) nLineNum=15;
			if (pJobItem->m_nDestPos == IMS_ECS_RET_STATION_256) nLineNum=21;
			if (pJobItem->m_nDestPos == IMS_ECS_RET_STATION_257) nLineNum=22;
		}
...
void CConveyor::CvDataChange1GroupTo7Group()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == 0 && m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_bStoStationReady) 
		{
			...
			nLuggNum = TRACK_INFO[nRearTrackNum].m_nLuggNum;
			nJobType = TRACK_INFO[nRearTrackNum].m_nJobType;
			nStartPos = TRACK_INFO[nRearTrackNum].m_nStartPos;
			nDestPos = TRACK_INFO[nRearTrackNum].m_nDestPos;
			nComplete = TRACK_INFO[nRearTrackNum].m_nLastPallet;
			...
			if (pMoveJobItem != NULL)
			{
				...
				//임시방편 
				if(m_pDoc->m_bSetTestCheck == TRUE)
				{
...
void CConveyor::CvDataChange7GroupTo8Group()
{
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == 0 && m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_bStoStationReady) 
		{
			...
			nStartPos = TRACK_INFO[nRearTrackNum].m_nStartPos;
			nDestPos = TRACK_INFO[nRearTrackNum].m_nDestPos;
			nComplete = TRACK_INFO[nRearTrackNum].m_nComplete;
...
/*			nLuggNum = TRACK_INFO[nFrontTrackNum].m_nLuggNum  = TRACK_INFO[nRearTrackNum].m_nLuggNum;
			...
			nStartPos = TRACK_INFO[nRearTrackNum].m_nStartPos;
			nDestPos = TRACK_INFO[nRearTrackNum].m_nDestPos;
			int nLastPallet = TRACK_INFO[nRearTrackNum].m_nLastPallet;
			...
			nStartPos = 2;
			nDestPos = 20;
			//nDestPos = 13; //신규 버전 적용 때 20 으로 변경 

			//if(m_pDoc->m_bWriteStackingCheck2 == TRUE)
			//	nDestPos = 12; //신규 버전 적용 때 20 으로 변경
...
void CConveyor::CvDataChange1GroupTo9Group()
{
	CString strLog;

	for (int i=0; i<1; i++)
	{
		int nFrontTrackNum = 2101 - 1;
		int nRearTrackNum  = 51 -1;

		if(m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos < 1 || m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos > 14)
			continue;
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == 0 && m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_bStoStationReady) 
		{
			...
			if (pMoveJobItem != NULL)
			{
				...
				if (WriteTrackInfo7(nFrontTrackNum, nLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, nSize))
				{
					strLog.Format("CvDataChange1GroupTo9Group.. 트랙[%d]->트랙[%d] 쓰기 성공..!", nRearTrackNum+1, nFrontTrackNum+1);
					LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
				}
				else
				{
					strLog.Format("CvDataChange1GroupTo9Group.. 트랙[%d]->트랙[%d] 쓰기 실패..!", nRearTrackNum+1, nFrontTrackNum+1);
					LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
				}
			}
		}
...
void CConveyor::CvDataChange1GroupTo9Group2()
{
	CString strLog;

	for (int i=0; i<1; i++)
	{
		int nFrontTrackNum = 2102 - 1;
		int nRearTrackNum  = 37 -1;
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == 0 && m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_bStoStationReady) 
		{
			...
			if (pMoveJobItem != NULL)
			{
				...
				if (WriteTrackInfo7(nFrontTrackNum, nLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, nSize))
				{
					strLog.Format("CvDataChange1GroupTo9Group2.. 트랙[%d]->트랙[%d] 쓰기 성공..!", nRearTrackNum+1, nFrontTrackNum+1);
					LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
				}
				else
				{
					strLog.Format("CvDataChange1GroupTo9Group2.. 트랙[%d]->트랙[%d] 쓰기 실패..!", nRearTrackNum+1, nFrontTrackNum+1);
					LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
				}
			}
		}
...
void CConveyor::CvDataChange8GroupTo9Group()
{
	CString strLog;

	for (int i=0; i<1; i++)
	{
		int nFrontTrackNum = 2114 - 1;
		int nRearTrackNum  = 2011 -1;

		if((m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos < 1 || m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos > 14)
			&& (m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos != 21 && m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos != 22))
			continue;
...
/*			nLuggNum = TRACK_INFO[nFrontTrackNum].m_nLuggNum  = TRACK_INFO[nRearTrackNum].m_nLuggNum;
			...
			if (WriteTrackInfo7(nFrontTrackNum, nLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, nSize))
			{
				strLog.Format("CvDataChange8GroupTo9Group.. 트랙[%d]->트랙[%d] 쓰기 성공..!", nRearTrackNum+1, nFrontTrackNum+1);
				LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
			}
			else
			{
				strLog.Format("CvDataChange8GroupTo9Group.. 트랙[%d]->트랙[%d] 쓰기 실패..!", nRearTrackNum+1, nFrontTrackNum+1);
				LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
			}
		}
	}	
...
void CConveyor::CvDataChange8GroupTo9Group2()
{
	CString strLog;

	for (int i=0; i<1; i++)
	{
		int nFrontTrackNum = 2109 - 1;
		int nRearTrackNum  = 2007 -1;

		if(m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos < 9 || m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos > 13)
			continue;
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == 0 && m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_bStoStationReady) 
		{
			...
			if (WriteTrackInfo7(nFrontTrackNum, nLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, nSize))
			{
				strLog.Format("CvDataChange8GroupTo9Group2.. 트랙[%d]->트랙[%d] 쓰기 성공..!", nRearTrackNum+1, nFrontTrackNum+1);
				LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
			}
			else
			{
				strLog.Format("CvDataChange8GroupTo9Group2.. 트랙[%d]->트랙[%d] 쓰기 실패..!", nRearTrackNum+1, nFrontTrackNum+1);
				LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
			}
		}
	}	
...
void CConveyor::CvDataChange9GroupTo8Group() //임시방편 
{
	CString strLog;

	for (int i=0; i<1; i++)
	{
		int nFrontTrackNum = 2007 - 1;
		int nRearTrackNum  = 2109 -1;

		if(m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nDestPos != 8)  
			continue;
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == 0 && m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_bStoStationReady) 
		{
			...
			if (WriteTrackInfo7(nFrontTrackNum, nLuggNum, 0, nStartPos, nDestPos, nJobType, nComplete, nSize))
			{
				strLog.Format("CvDataChange9GroupTo8Group.. 트랙[%d]->트랙[%d] 쓰기 성공..!", nRearTrackNum+1, nFrontTrackNum+1);
				LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
			}
			else
			{
				strLog.Format("CvDataChange9GroupTo8Group.. 트랙[%d]->트랙[%d] 쓰기 실패..!", nRearTrackNum+1, nFrontTrackNum+1);
				LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
			}
		}
	}	
...
void CConveyor::RemoveCvData8GroupTo9Group()
{
	CString strLog;
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nLuggNum )  
		{
			...
			{
				strLog.Format("RemoveCvData8GroupTo9Group.. 트랙[%d] 데이타 삭제 성공..!", nRearTrackNum+1);
				LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
			}
			else
			{
				strLog.Format("RemoveCvData8GroupTo9Group.. 트랙[%d] 데이타 삭제 실패..!", nRearTrackNum+1);
				LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
			}
		}
	}	
...
void CConveyor::RemoveCvData9GroupTo8Group() //임시방편 
{
	CString strLog;
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nLuggNum )  
		{
			if(WriteTrackInfo7(nRearTrackNum, 0, 0, 0, 0, 0))
			{
				strLog.Format("RemoveCvData9GroupTo8Group.. 트랙[%d] 데이타 삭제 성공..!", nRearTrackNum+1);
				LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
			}
			else
			{
				strLog.Format("RemoveCvData9GroupTo8Group.. 트랙[%d] 데이타 삭제 실패..!", nRearTrackNum+1);
				LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
			}
		}
	}	
...
void CConveyor::RemoveCvData1Group2()
{
	CString strLog;
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nLuggNum )  
		{
			if (WriteTrackInfo(nRearTrackNum, 0, 0, 0, 0, 0))
			{
				strLog.Format("RemoveCvData1Group2.. 트랙[%d] 데이타 삭제 성공..!", nRearTrackNum+1);
				LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
			}
			else
			{
				strLog.Format("RemoveCvData1Group2.. 트랙[%d] 데이타 삭제 실패..!", nRearTrackNum+1);
				LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
			}
		}
	}	
...
void CConveyor::RemoveCvData8Group2()
{
	CString strLog;
	...
	for (int i=0; i<1; i++)
	{
		...
		if(m_pDoc->m_ConveyorTrackInfo[nFrontTrackNum].m_nLuggNum == m_pDoc->m_ConveyorTrackInfo[nRearTrackNum].m_nLuggNum )  
		{
			if (WriteTrackInfo7(nRearTrackNum, 0, 0, 0, 0, 0))
			{
				strLog.Format("RemoveCvData8Group2.. 트랙[%d] 데이타 삭제 성공..!", nRearTrackNum+1);
				LOG_JOB(LOG_POS_CV, nLuggNum, strLog);
			}
			else
			{
				strLog.Format("RemoveCvData8Group2.. 트랙[%d] 데이타 삭제 실패..!", nRearTrackNum+1);
				LOG_ERROR(LOG_POS_CV, nLuggNum, strLog);
			}
		}
	}	
...
//		if(pSecondJobItem != NULL)		continue;
		...
		if(pJobItem->m_nDestPos == IMS_ECS_RET_STATION_201)	nDestPos = CV_RET_STATION_4GROUP;
			    ...
			    pJobItem->m_nDestPos == IMS_ECS_RET_STATION_256 || 
			    pJobItem->m_nDestPos == IMS_ECS_RET_STATION_257)	nDestPos = 4;
		else 0;
...
//	sprintf((char *)TxBuff, "%.04d%2d%2x%2d%1d%1d%", nLuggNum, nStartPos, nDestPos, nComplete, nSize, nJobType);
	...
	if(	nTrackNum >= 2100)
		nStartAddr = ((nTrackNum+1-2100) * 10);
...
int CConveyor::WriteLineGroupInfo(int nD1, int nD2, int nD3, int nD4, int nD5, int nD6, int nD7, int nD8, int nD9, int nD10, int nD11, int nD12, int nD13, int nD14)
{
	BYTE	TxBuff[256];
	memset(TxBuff, 0x00, sizeof(TxBuff));

    TxBuff[0] = (BYTE)(nD1 & 0xFF);
    TxBuff[2] = (BYTE)(nD2 & 0xFF);
    TxBuff[4] = (BYTE)(nD3 & 0xFF);
    TxBuff[6] = (BYTE)(nD4 & 0xFF);
    TxBuff[8] = (BYTE)(nD5 & 0xFF);
    TxBuff[10] = (BYTE)(nD6 & 0xFF);
    TxBuff[12] = (BYTE)(nD7 & 0xFF);
    TxBuff[14] = (BYTE)(nD8 & 0xFF);
    TxBuff[16] = (BYTE)(nD9 & 0xFF);
    TxBuff[18] = (BYTE)(nD10 & 0xFF);
    TxBuff[20] = (BYTE)(nD11 & 0xFF);
    TxBuff[22] = (BYTE)(nD12 & 0xFF);
    TxBuff[24] = (BYTE)(nD13 & 0xFF);
    TxBuff[26] = (BYTE)(nD14 & 0xFF); 

	return Write(CMD_WORD_UNIT, TxBuff, DEVICE_CODE_D,  2560, 14);
}
