...
#define ECS_STATION_CNT					4 + 18
...

	{
		...
		//			CV_TRACK_INFO[8-1].m_bStoStationReady);
		sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT), "%d",
		...
		sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3, "%d",
					1/*!m_pDoc->m_bStoRetMode*/); //없어짐. 예전엔 수동창고 가능상태가 IMS에선 1이다 

		if(m_pDoc->m_bAllStackerEmergency == TRUE)
		{
			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 1, "%d",
						0/*CV_TRACK_INFO[904-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 2, "%d",
						0/*CV_TRACK_INFO[910-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 3, "%d",
						0/*CV_TRACK_INFO[919-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 4, "%d",
						0/*CV_TRACK_INFO[1013-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 5, "%d",
						0/*CV_TRACK_INFO[1030-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 6, "%d",
						0/*CV_TRACK_INFO[1018-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 7, "%d",
						0/*CV_TRACK_INFO[926-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 8, "%d",
						0/*CV_TRACK_INFO[1032-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 9, "%d",
						0/*CV_TRACK_INFO[933-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 10, "%d",
						0/*CV_TRACK_INFO[939-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 11, "%d",
						0/*CV_TRACK_INFO[945-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 12, "%d",
						0/*CV_TRACK_INFO[952-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 13, "%d",
						0/*CV_TRACK_INFO[960-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 14, "%d",
						0/*CV_TRACK_INFO[1034-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 15, "%d",
						0/*CV_TRACK_INFO[1036-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 16, "%d",
						0/*CV_TRACK_INFO[966-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 17, "%d",
						0/*CV_TRACK_INFO[1038-1].m_bStoStationReady*/);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 18, "%d",
						0/*CV_TRACK_INFO[973-1].m_bStoStationReady*/);
		}
		else
		{
			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 1, "%d",
						CV_TRACK_INFO[904-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 2, "%d",
						CV_TRACK_INFO[910-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 3, "%d",
						CV_TRACK_INFO[919-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 4, "%d",
						CV_TRACK_INFO[1013-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 5, "%d",
						CV_TRACK_INFO[1030-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 6, "%d",
						CV_TRACK_INFO[1018-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 7, "%d",
						CV_TRACK_INFO[926-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 8, "%d",
						CV_TRACK_INFO[1032-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 9, "%d",
						CV_TRACK_INFO[933-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 10, "%d",
						CV_TRACK_INFO[939-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 11, "%d",
						CV_TRACK_INFO[945-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 12, "%d",
						CV_TRACK_INFO[952-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 13, "%d",
						CV_TRACK_INFO[960-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 14, "%d",
						CV_TRACK_INFO[1034-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 15, "%d",
						CV_TRACK_INFO[1036-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 16, "%d",
						CV_TRACK_INFO[966-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 17, "%d",
						CV_TRACK_INFO[1038-1].m_bStoStationReady);

			sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + 3 + 18, "%d",
						CV_TRACK_INFO[973-1].m_bStoStationReady);
		}
	}	

    //IMS 모니터 Display를 위해..
	{
		sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + ECS_STATION_CNT, "%.4d",
			CV_TRACK_INFO[9-1].m_nLuggNum);
		
		sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + ECS_STATION_CNT + 4, "%.4d",
			CV_TRACK_INFO[901-1].m_nLuggNum);
		
		sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + ECS_STATION_CNT + 8, "%.4d",
			CV_TRACK_INFO[902-1].m_nLuggNum);
		
		sprintf(TxBuff + MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + ECS_STATION_CNT + 12, "%.4d",
			CV_TRACK_INFO[1021-1].m_nLuggNum);
	}


	//TxBuff[MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + ECS_STATION_CNT] = ETX;
	TxBuff[MSG_LENGTH_HEADER + 2 + (11 * SC_CNT) + ECS_STATION_CNT + 16] = ETX;
...
void CHostCl::Arrived3(int nLuggNum, int nLuggNum2)
{
	CMoveJobItem* pMoveJobItem = MOVEJOB->Find(nLuggNum);
	if (pMoveJobItem == NULL)
	{
...
//	}
	...
	if(SendMsg(TxBuff, MSG_LENGTH_ARRIVAL_MSG) == FALSE)
	{
		strLog.Format("도착완료 보고 송신 실패3.. 에러=[%s]", CLib::GetSystemErrMsg());
		LOG_ERROR(LOG_POS_HOST, nLuggNum, strLog);
...
//		LOG_ERROR(LOG_POS_HOST, nLuggNum2, strLog);
	...
	strLog.Format("도착완료 보고3.. JobType=[%s] Station=[%d]", pMoveJobItem->GetType(), pMoveJobItem->m_nDestPos);
	LOG_JOB(LOG_POS_HOST, nLuggNum, strLog);

//	strLog.Format("도착완료 보고3.. JobType=[%s] Station=[%d]", pMoveJobItem2->GetType(), pMoveJobItem2->m_nDestPos);
//	LOG_JOB(LOG_POS_HOST, nLuggNum2, strLog);
	...
	case	CMD_LOAD_ARRV_ANSWER:
		{
			...
			strLog.Format("CHostCl::Parsing.. 도착 완료 응답 수신.. Lugg=[%d]", nLuggNum);
			m_pDoc->m_pLog->Job(LOG_POS_HOST, nLuggNum, strLog);
			...
			if(nLuggNum2 != 0)
			{
				...
				strLog.Format("CHostCl::Parsing.. 도착 완료 응답 수신.. Lugg=[%d]", nLuggNum2);
