...
void CConveyorDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_PLC_NUM, m_strPlcNum);
...
*/
	...
	strTemp.Format("%02d", nJobType);
	if (nJobType == -1) strTemp = "0";
	GetDlgItem(IDC_EDIT_JOB_TYP)->SetWindowText(strTemp);
	...
	strTemp.Format("%02d", m_pDoc->m_ConveyorTrackInfo[m_nID].m_nStartPos);
	if (m_pDoc->m_ConveyorTrackInfo[m_nID].m_nStartPos == -1) strTemp = "0";
	GetDlgItem(IDC_EDIT_STARTNATION)->SetWindowText(strTemp);
...
*/
	...
	strTemp.Format("%02d", m_pDoc->m_ConveyorTrackInfo[m_nID].m_nDestPos);
	if (m_pDoc->m_ConveyorTrackInfo[m_nID].m_nDestPos == -1) strTemp = "0";
	GetDlgItem(IDC_EDIT_DESTINATION)->SetWindowText(strTemp);
	...
	//SET(IDC_EDIT_PLC_NUM, m_pDoc->m_pConveyorThread->
